const UserCard = ({ name, role, imageUrl }) => {
    ...
    if (imageUrl) {
        avatar = <img className="w-16 h-16 rounded-full" src={imageUrl} alt="Profile image" />
    }
    ...
    return (
        <div className="max-w-md bg-white mb-2 rounded-xl p-6 shadow-lg flex flex-row items-center border-gray-100 border">
            ...
            <div className="ml-5">
                ...
                <button className="rounded-full border-blue-600 hover:bg-blue-600 hover:text-white text-blue-600 mt-2 px-6 border shadow-sm p-2 text-sm font-bold">Details</button>
            </div>
        </div>
    );
}
